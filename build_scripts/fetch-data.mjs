...
const FIELD_MAP = {
  ...
  "Total Block 1 Access Points": "block1_total",
  "Block 1 Access Restriction Description": "block1_desc",
...
const remapped = rows
    .filter(row => row['CARSO status']?.trim() !== 'Exclude') // exclude entirely
    .map(row => {
      const mapped = {};
      for (const [originalKey, shortKey] of Object.entries(FIELD_MAP)) {
        if (row.hasOwnProperty(originalKey)) {
          ...
          if (typeof value === 'string') {
            value = value.trim();
            if (['n/a', '0', 'No Icon'].includes(value)) {
              value = null;
            }
          }
